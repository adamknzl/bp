/**
 * @file  categories.ts
 * @brief Mapping thematic category names to Tailwind color classes.
 * @author Adam Kinzel (xkinzea00)
 *
 * Category names must match the values stored in the database exactly.
 * Unknown names fall back to the "Ostatní" entry.
 */
...
/**
 * Return the Tailwind color classes for a category name.
 * Falls back to the "Ostatní" entry for unrecognised names.
 */
export const getCategoryColor = (categoryName: string): string =>
  CATEGORY_COLORS[categoryName] ?? CATEGORY_COLORS['Ostatní'];