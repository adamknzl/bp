...
branch-linking feature is showcased in the database. Each guaranteed pair 
contributes one parent record and one branch record to the sample.
The remaining slots are filled with a random draw from the rest of the source file.
"""
...
def _find_pairs(parents: pd.DataFrame, branches: pd.DataFrame) -> list[tuple]:
    ...
    its name until a match is found in the parent name set.

    Example:
        Branch: "Český svaz včelařů, z.s., základní organizace Pláně"
        Parent: "Český svaz včelařů, z.s."
    """
    # Build name, index map for fast parent lookup
    ...
    for branch_idx, branch_name in branches['FIRMA'].items():
        ...
        # Trying all prefixes from longest to shortest
        for i in range(len(parts) - 1, 0, -1):
            candidate = ', '.join(parts[:i])
            if candidate in parent_name_to_idx:
                parent_idx = parent_name_to_idx[candidate]
                # One branch per parent to keep sample balanced
...
def sample_source(args) -> None:
    ...
    print(f"  Found {len(pairs)} parent-branch pairs.")
    ...
    # Shuffle to keep guaranteed entries from grouping at the top
